...
连接成功后，白藤作为同一个 App 的透明置顶窗口，不再单独启动 Electron 或安装 `pet` 目录的依赖；App 自己的开发依赖包含打包所需的 Pixi 运行库。
对话或连接窗口在前台时桌宠默认关闭；窗口失焦、最小化或 App 进入后台时自动显示。菜单 **Studio → 后台显示桌宠** 可关闭本次运行的该行为。
**找回桌宠位置** 将它移回主屏幕。按住人物或背景拖动窗口，
拖动任意一个角等比例缩放（40%–150%）。右上角可收起通话窗口；也可使用 **Studio → 桌宠大小** 恢复原始大小。
...
- 桌宠通过当前后端的 `/ws-pet` 只读观察真实播放进度、附和、打断和断线，不采集麦克风、不创建第二段对话；通话按钮调用当前主页面的语音开关。首次麦克风授权仍由主页面处理，App 最小化时通话可继续。
- 切换服务会关闭旧桌宠连接；关闭 Studio 主窗口或退出 App 会关闭内置桌宠，Docker 后端继续运行。
- 打包包含白藤 Cubism Live2D 模型、Pixi 运行库、程序化动作和竖屏通话背景；桌宠按钮复用主 App 的语音会话，不再显示数字动作面板。
...
- 桌宠只有受限窗口和对话开关接口，没有连接设置、Docker 或 Node 权限。资源请求限于内置文件和当前 `/ws-pet`；对话请求仅能触发当前主页面已有的语音按钮。
...
它加载实际 Live2D 人物和通话背景，用合成的 `/ws-pet` 事件验证嘴型、白藤动作、打断、暂停/断线、服务切换及隐藏窗口。
